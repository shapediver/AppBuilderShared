...
import {IAppBuilderActionPropsAddToCart} from "../config/appbuilder";

export interface UseAppBuilderActionAddToCartProps extends IAppBuilderActionPropsAddToCart {
	namespace: string;
	disabled?: boolean;
}

/** Logic for the "addToCart" action. Can be used without the action component. */
export function useAppBuilderActionAddToCart(
	props: UseAppBuilderActionAddToCartProps,
) {
	const {
		namespace,
		productId,
	...
	const {
		...
		screenshotProps: themeScreenshotProps,
	} = useProps(
		"AddToCartAction",
	...
	const trigger = useCallback(async () => {
		...
		try {
			...
			const resultModelState = await createModelState({
				...
				includeImage,
				image,
				screenshotProps: screenshotProps ?? themeScreenshotProps,
