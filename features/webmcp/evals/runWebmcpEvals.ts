...
function loadScenarios(): EvalScenario[] {
	const raw = readFileSync(join(__dirname, "evals.json"), "utf8");
...
function assertSchemaScenario(scenario: EvalScenario): string | null {
	...
	if (expect.success === true) {
		console.log(`  schema-valid (mocked execution)`);
	}

	return null;
}

async function runScenario(scenario: EvalScenario): Promise<string | null> {
...
async function main() {
	...
	for (const scenario of scenarios) {
		const reason = await runScenario(scenario);
