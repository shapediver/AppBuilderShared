...
describe("mapParameterDefinition", () => {
	it("maps STRINGLIST with index current/default values", () => {
		...
		expect(mapParameterDefinition(param, SESSION_ID)).toEqual({
			...
			name: "Material",
			type: ResParameterType.STRINGLIST,
			howto: 'Use a 0-based integer index (0..1). Choices: ["Wood","Metal"]. Never send the label text. Never wrap in {index:N}.',
			settable: true,
			choices: ["Wood", "Metal"],
			currentValue: 0,
			defaultValue: 1,
	...
	it("maps FLOAT with min/max/decimalplaces", () => {
		...
		expect(mapParameterDefinition(param, SESSION_ID)).toEqual({
			...
			name: "Width",
			type: ResParameterType.FLOAT,
			howto: "Use a number in range [0, 100].",
			settable: true,
			min: 0,
			max: 100,
			decimalplaces: 2,
	...
	it("includes group and tooltip from definition", () => {
		...
		expect(mapParameterDefinition(param, SESSION_ID)).toEqual({
			...
			name: "Width",
			type: ResParameterType.INT,
			howto: "Use an integer in range [0, 10].",
			settable: true,
			group: "Dimensions",
			tooltip: "Model width",
			min: 0,
