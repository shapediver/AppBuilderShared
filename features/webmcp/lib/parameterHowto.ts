...
export function howtoForParameterType(
	...
	if (type === ResParameterType.STRINGLIST) {
		...
		return `Use a 0-based integer index (0..${Math.max(choices.length - 1, 0)}). Choices: ${JSON.stringify(choices)}. Never send the label text. Never wrap in {index:N}. If choices look like numbers (e.g. ['4','6','8'] or ['3','3 1/4',...]) the value is still the INDEX, not the label — e.g. for choices ['4','6','8'], '6 prongs' = index 1, not value 6; for a label that is itself a number (e.g. '7'), find its position in choices and send that index, not the number itself. Match choice labels case-insensitively to find the index (e.g. 'Button' matches 'button' = index 0), then send the index.`;
	}
