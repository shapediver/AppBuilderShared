...
export function useSessionWithCustomUi(props: IUseSessionDto | undefined) {
	...
	const {sessionApi} = useSession(
		props
			? {
					...
					acceptRejectMode: (param) =>
						param.name === CUSTOM_DATA_INPUT_NAME
							? false
							: !!props.acceptRejectMode,
				}
			: undefined,
	);
	...
	useEffect(() => {
		if (sessionApi)
			console.debug(
				`Available output names for session ${sessionApi.id}: ${Object.values(sessionApi.outputs).map((o) => o.name)}`,
			);
	}, [sessionApi]);
	...
	// custom UI state
	const [customUiState, setCustomUiState] = useState<ICustomUiState>({
		hidden: [],
		parameters: [],
	});

	// custom UI parameter
	const customUiParam = useParameterStateless<string>(
		namespace,
	...
	const executor = useCallback<IGenericParameterExecutor>(
		async (values) =>
			new Promise((resolve, reject) => {
				...
				if (
					customUiParam &&
					customUiParam.definition.type === "String"
				) {
					customUiParam.actions.setUiValue(
						JSON.stringify(customParameterValues.current),
					);
					customUiParam.actions
						.execute(true)
						.then(resolve)
						.catch(reject);
				} else {
					console.warn(
						`Could not find a string input named ${CUSTOM_DATA_INPUT_NAME}`,
					);
					resolve(values);
				}
			}),
	...
	useEffect(() => {
		if (outputApi) {
			if (
				...
				console.debug(
					`Output ${outputApi?.id} (${outputApi?.displayname ? outputApi?.displayname : outputApi?.name}) version ${outputApi?.version}`,
					customUiData,
				);

				customParameterValues.current = {};
				customUiData.parameters?.forEach(
					(p) => (customParameterValues.current[p.id] = p.defval),
				);
				setCustomUiState({
					hidden: customUiData.hidden || [],
					parameters: (customUiData.parameters || []).map((p) => {
						return {definition: p};
					}),
				});
			} else
				console.debug(
					`Output with name "${CUSTOM_DATA_OUTPUT_NAME}" does not contain expected custom UI data`,
				);
		} else
			console.debug(
	...
	const sessionParameterProps = useSessionPropsParameter(
		...
		(param) => {
			...
			if (
				...
				CUSTOM_DATA_INPUT_NAME.toLowerCase()
			)
				return false;
