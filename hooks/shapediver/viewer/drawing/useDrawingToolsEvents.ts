...
export function useDrawingToolsEvents(
	...
	useEffect(() => {
		 ...
		 */
		const tokenDrawingToolsMinimumPoints = addListener(
			EVENTTYPE_DRAWING_TOOLS.MINIMUM_POINTS,
			async (event: IEvent) => {
				const drawingToolsEvent =
					event as DrawingToolsEventResponseMapping[EVENTTYPE_DRAWING_TOOLS.MINIMUM_POINTS];
		 ...
		 * In this event handler, a notification is shown.
		 */
		const tokenDrawingToolsMaximumPoints = addListener(
			EVENTTYPE_DRAWING_TOOLS.MAXIMUM_POINTS,
			async (event: IEvent) => {
				const drawingToolsEvent =
					event as DrawingToolsEventResponseMapping[EVENTTYPE_DRAWING_TOOLS.MAXIMUM_POINTS];
		...
		const tokenDrawingToolsGeometryChanged = addListener(
			...
			(e: IEvent) => {
				...
				if (
					drawingToolsEvent.temporary === false &&
					drawingToolsEvent.points !== undefined &&
					drawingToolsEvent.recordHistory !== false
				) {
