...
const radiusSizes: Record<MantineSize, boolean> = { "xs": true, "sm": true, "md": true, "lg": true, "xl": true };
...
export default function Svg(props: SvgProps) {
	...
	useEffect(() => {
		fetch(srcUrl).then(res => res.text()).then((svgString) => {
			const parser = new DOMParser();
			const svgDoc = parser.parseFromString(svgString, "image/svg+xml");
			const element = svgDoc.documentElement;

			element.setAttribute("width", w !== undefined ? String(w) : "auto");
			element.setAttribute("height", h !== undefined ? String(h) : "auto");
